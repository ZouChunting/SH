...
//验证表单方式一
//function checkForm(){
//    var ids  = ["checkUserName()","checkPwd()","checkRepwd()","checkNikeName()","checkTel()","checkEmail()","checkzsName"];
//    var sum = 0;
...
//验证表单方式二
function checkForm(){
	if(checkUserName()&checkPwd()&checkRepwd()&checkNikeName()&checkTel()&checkEmail()&checkzsName()){
		return true;
	}else{
		return false;
	}
...
function checkUserName(){
  ...
  //}else if( reg.test(odom.value)==false){
        odom.style.borderColor = "red";
...
//验证真实姓名   
function checkzsName(){
	var flag = true;
    var odom = e("zsName");//输入框DOM对象
    var omsg = e("zsNameId");//信息提示DOM对象
	var reg= new RegExp("^([\u4E00-\u9FA5]+|[a-zA-Z]+)$");
	if(odom.value == ""){
		odom.style.borderColor = "red";
        appendHtml(omsg, "真实姓名不能为空,请输入昵称", "error_prompt");
        flag = false;
	}else if(odom.value.match(reg)==null){
		odom.style.borderColor = "red";
        appendHtml(omsg, "姓名可以是纯英文或纯汉字,不能出现特殊字符", "error_prompt");
        flag = false;
	}else{
		odom.style.borderColor = "green";
        appendHtml(omsg,"输入正确","ok_prompt");
